...
<body>
    <table cellspacing="0" border="0" cellpadding="0">
        <tr>
            ...
            <td>
                <h3 class="text-end"># {{ $laporan->pembayaran->no_invoice }}</h3>
            </td>
        </tr>
    </table>
        ...
        <table>
            <tr>
                <td>
                    ...
                    <h4 style="margin: 0">{{ $laporan->servis->booking->user->name }}</h4>
                    <h4 style="margin: 0">{{ $laporan->servis->booking->user->no_telepon }}</h4>
                </td>
                <td>
                    <div>Alamat : </div>
                    <h4 style="margin: 0">{{ $laporan->servis->booking->alamat_lengkap }}</h4>
                    <h4 style="margin: 0">{{ $laporan->servis->booking->kecamatan->nm_kecamatan }}, {{ $laporan->servis->booking->kota->nm_kota }}</h4>
                </td>
            </tr>
        </table>
    <table>
        <tr>
            <td>
                <div style="margin: 2rem 0">
                    ...
                    <div>
                        @foreach ($laporan->servis->booking->kendalas as $kendala)
            ...
            <td>
                <div style="margin: 2rem 0">
                    ...
                    <div>
                        @if (isset($laporan->servis->booking->catatan))
                        <strong>{{ $laporan->servis->booking->catatan }}</strong>
                        @else
                        <strong>-</strong>
                        @endif
                    </div>
            ...
            <td>
                <div style="margin: 2rem 0">
                    ...
                    <div>
                        <strong>{{ $laporan->servis->teknisi->nama_teknisi }}</strong>
                    </div>
                </div>
            </td>
        </tr>
                        ...
                        <tr>
                            ...
                            <td class="text-end">
                                ...
                                <span>Biaya Servis : Rp. 
                                    {{ number_format($servis = $laporan->servis->harga, 0, ',', '.') }}
                                </span>
                                <br><br>
                                <span>Kode Unik : 
                                    {{ $last_digit }}
                                ...
                                <br><br>
                                <span>Ongkir : Rp.
                                    {{ number_format($ongkir = $laporan->servis->booking->kecamatan->hrg_ongkir, 0, ',', '.') }}
                                </span>
                            </td>
                        </tr>
                    <tr>
    ...
    <div style="margin-top: 3em">
        <div>Dengan ini, Servis AC atas nama {{ $laporan->servis->booking->user->name }} telah selesai dengan Total Biaya : {{ number_format(@$sparepart->sum('total') + $servis + $ongkir + $last_digit, 0, ',', '.') }}</div>
    </div>
</body>
